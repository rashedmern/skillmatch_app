...
export const PipelineStep: React.FC<PipelineStepProps> = ({ step, isLast }) => {
  ...
  return (
    <div className="relative flex-1 flex flex-col items-center">
      {/* Step Card Container */}
      <div className="relative w-full rounded-2xl bg-surface-container-lowest border border-stroke-card p-6 sm:p-8 shadow-level-1 hover:shadow-level-2 hover:border-secondary-mint/40 transition-all duration-200 flex flex-col justify-between h-full z-10 group">
        <div>
          {/* Top Header: Step Number Pill + Step Icon */}
          <div className="flex items-center justify-between gap-4 mb-6">
            <span className="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-bold font-mono tracking-wider bg-surface-container-low border border-stroke-card text-primary-container tabular-nums">
              STEP {step.stepNumber}
            </span>

            <div className="w-12 h-12 rounded-xl bg-secondary/10 border border-secondary/25 flex items-center justify-center group-hover:scale-105 group-hover:bg-secondary/15 transition-all duration-200">
          ...
          <div className="space-y-1">
            <span className="text-[11px] font-bold uppercase tracking-wider text-secondary-mint">
              {step.subtitle}
            </span>
            <h3 className="text-xl font-bold text-on-surface font-sans leading-snug group-hover:text-primary-container transition-colors">
              {step.title}
            </h3>
          </div>

          {/* Description */}
          <p className="text-xs sm:text-sm text-on-surface-variant font-normal leading-relaxed mt-3 font-sans">
            {step.description}
          </p>
        </div>
